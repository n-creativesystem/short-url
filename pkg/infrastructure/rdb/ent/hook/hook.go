...
// The ShortsFunc type is an adapter to allow the use of ordinary
// function as Shorts mutator.
type ShortsFunc func(context.Context, *ent.ShortsMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f ShortsFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.ShortsMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.ShortsMutation", m)
}
